...
if [ ! -f "$SSH_KEY" ]; then
    ...
    echo "   Options:"
    echo "   1. Set SSH_KEY env var: SSH_KEY=~/.ssh/my-key ./hetzner_deploy/scripts/push-certs.sh"
    echo "   2. Update ssh_public_key_path in hetzner_deploy/terraform/environments/prod/terraform.tfvars"
    exit 1
fi
...
mkdir -p "$SSL_DIR"
terraform output -raw sinkai_origin_certificate > "$SSL_DIR/cloudflare-origin.pem"
terraform output -raw sinkai_origin_private_key > "$SSL_DIR/cloudflare-origin.key"
chmod 600 "$SSL_DIR/cloudflare-origin.key"

echo ""
echo "🚀 Pushing certs to root@$SERVER_IP:/opt/sinkai/ssl/ ..."
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=accept-new "root@$SERVER_IP" "mkdir -p /opt/sinkai/ssl"
scp -i "$SSH_KEY" "$SSL_DIR/cloudflare-origin.pem" "$SSL_DIR/cloudflare-origin.key" "root@$SERVER_IP:/opt/sinkai/ssl/"
ssh -i "$SSH_KEY" "root@$SERVER_IP" "chmod 600 /opt/sinkai/ssl/cloudflare-origin.key && chmod 644 /opt/sinkai/ssl/cloudflare-origin.pem"
