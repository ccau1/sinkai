...
```bash
cp hetzner_deploy/.env.example hetzner_deploy/.env
# Edit hetzner_deploy/.env with your values
```
...
```bash
./hetzner_deploy/scripts/setup-server.sh
```
...
```bash
./hetzner_deploy/scripts/deploy.sh
```

### 4. Health check (optional)

```bash
./hetzner_deploy/scripts/health-check.sh
```
...
1. **Copy the folder:**
   ```bash
   cp -r hetzner_deploy/ /path/to/other-repo/
   ```
...
For prod, the server and DNS are managed by Terraform in `hetzner_deploy/terraform/environments/prod/`.

📖 **See [`hetzner_deploy/terraform/README.md`](terraform/README.md) for full Terraform docs:** environment structure, state isolation, adding new environments, and destroy safety.
...
```bash
./hetzner_deploy/scripts/push-certs.sh
```
