...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`media\` (
  	...
  	\`alt_zh_c_n\` text NOT NULL,
  	\`alt_zh_t_w\` text NOT NULL,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`url\` text,
  ...
  await db.run(sql`CREATE TABLE \`installations\` (
  	...
  	\`location_zh_c_n\` text NOT NULL,
  	\`location_zh_t_w\` text NOT NULL,
  	\`completion_date\` text,
  	\`description_en\` text,
  	\`description_zh_c_n\` text,
  	\`description_zh_t_w\` text,
  ...
  await db.run(sql`CREATE INDEX \`installations_rels_media_id_idx\` ON \`installations_rels\` (\`media_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`blogs_rels_installations_id_idx\` ON \`blogs_rels\` (\`installations_id\`);`)
