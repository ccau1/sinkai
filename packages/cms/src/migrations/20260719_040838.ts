...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`installations_locales\` (
  	\`title\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`installations_meta_meta_image_idx\` ON \`installations_locales\` (\`meta_image_id\`,\`_locale\`);`)
  await db.run(sql`CREATE UNIQUE INDEX \`installations_locales_locale_parent_id_unique\` ON \`installations_locales\` (\`_locale\`,\`_parent_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`blogs_meta_meta_image_idx\` ON \`blogs_locales\` (\`meta_image_id\`,\`_locale\`);`)
  ...
  await db.run(sql`CREATE TABLE \`navigation_items\` (
  	...
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`link_type\` text DEFAULT 'page' NOT NULL,
  	\`page_id\` integer,
  	\`path\` text,
  	\`url\` text,
  ...
  // Migrate existing suffix-localized data into native Payload locale rows.
  await db.run(sql`INSERT INTO \`media_locales\` (\`alt\`, \`_locale\`, \`_parent_id\`) SELECT \`alt_en\`, 'en', \`id\` FROM \`media\`;`)
  await db.run(sql`INSERT INTO \`media_locales\` (\`alt\`, \`_locale\`, \`_parent_id\`) SELECT \`alt_zh_c_n\`, 'zh-CN', \`id\` FROM \`media\`;`)
  await db.run(sql`INSERT INTO \`media_locales\` (\`alt\`, \`_locale\`, \`_parent_id\`) SELECT \`alt_zh_t_w\`, 'zh-TW', \`id\` FROM \`media\`;`)

  await db.run(sql`INSERT INTO \`installations_locales\` (\`title\`, \`location\`, \`description\`, \`_locale\`, \`_parent_id\`) SELECT \`title_en\`, \`location_en\`, \`description_en\`, 'en', \`id\` FROM \`installations\`;`)
  await db.run(sql`INSERT INTO \`installations_locales\` (\`title\`, \`location\`, \`description\`, \`_locale\`, \`_parent_id\`) SELECT \`title_zh_c_n\`, \`location_zh_c_n\`, \`description_zh_c_n\`, 'zh-CN', \`id\` FROM \`installations\`;`)
  await db.run(sql`INSERT INTO \`installations_locales\` (\`title\`, \`location\`, \`description\`, \`_locale\`, \`_parent_id\`) SELECT \`title_zh_t_w\`, \`location_zh_t_w\`, \`description_zh_t_w\`, 'zh-TW', \`id\` FROM \`installations\`;`)

  await db.run(sql`INSERT INTO \`blogs_locales\` (\`slug_name\`, \`title\`, \`excerpt\`, \`content\`, \`legacy_content\`, \`_locale\`, \`_parent_id\`) SELECT \`slug_name_en\`, \`title_en\`, \`excerpt_en\`, \`content_en\`, \`legacy_content_en\`, 'en', \`id\` FROM \`blogs\`;`)
  await db.run(sql`INSERT INTO \`blogs_locales\` (\`slug_name\`, \`title\`, \`excerpt\`, \`content\`, \`legacy_content\`, \`_locale\`, \`_parent_id\`) SELECT \`slug_name_zh_c_n\`, \`title_zh_c_n\`, \`excerpt_zh_c_n\`, \`content_zh_c_n\`, \`legacy_content_zh_c_n\`, 'zh-CN', \`id\` FROM \`blogs\`;`)
  await db.run(sql`INSERT INTO \`blogs_locales\` (\`slug_name\`, \`title\`, \`excerpt\`, \`content\`, \`legacy_content\`, \`_locale\`, \`_parent_id\`) SELECT \`slug_name_zh_t_w\`, \`title_zh_t_w\`, \`excerpt_zh_t_w\`, \`content_zh_t_w\`, \`legacy_content_zh_t_w\`, 'zh-TW', \`id\` FROM \`blogs\`;`)

  await db.run(sql`ALTER TABLE \`media\` DROP COLUMN \`alt_en\`;`)
  await db.run(sql`ALTER TABLE \`media\` DROP COLUMN \`alt_zh_c_n\`;`)
  await db.run(sql`ALTER TABLE \`media\` DROP COLUMN \`alt_zh_t_w\`;`)
  await db.run(sql`ALTER TABLE \`installations\` DROP COLUMN \`title_en\`;`)
  await db.run(sql`ALTER TABLE \`installations\` DROP COLUMN \`location_en\`;`)
  await db.run(sql`ALTER TABLE \`installations\` DROP COLUMN \`description_en\`;`)
  await db.run(sql`ALTER TABLE \`installations\` DROP COLUMN \`title_zh_c_n\`;`)
  await db.run(sql`ALTER TABLE \`installations\` DROP COLUMN \`location_zh_c_n\`;`)
  await db.run(sql`ALTER TABLE \`installations\` DROP COLUMN \`description_zh_c_n\`;`)
  await db.run(sql`ALTER TABLE \`installations\` DROP COLUMN \`title_zh_t_w\`;`)
  await db.run(sql`ALTER TABLE \`installations\` DROP COLUMN \`location_zh_t_w\`;`)
  await db.run(sql`ALTER TABLE \`installations\` DROP COLUMN \`description_zh_t_w\`;`)
  await db.run(sql`ALTER TABLE \`blogs\` DROP COLUMN \`slug_name_en\`;`)
  await db.run(sql`ALTER TABLE \`blogs\` DROP COLUMN \`title_en\`;`)
  await db.run(sql`ALTER TABLE \`blogs\` DROP COLUMN \`excerpt_en\`;`)
  await db.run(sql`ALTER TABLE \`blogs\` DROP COLUMN \`content_en\`;`)
  await db.run(sql`ALTER TABLE \`blogs\` DROP COLUMN \`legacy_content_en\`;`)
  await db.run(sql`ALTER TABLE \`blogs\` DROP COLUMN \`slug_name_zh_c_n\`;`)
  await db.run(sql`ALTER TABLE \`blogs\` DROP COLUMN \`title_zh_c_n\`;`)
  await db.run(sql`ALTER TABLE \`blogs\` DROP COLUMN \`excerpt_zh_c_n\`;`)
  await db.run(sql`ALTER TABLE \`blogs\` DROP COLUMN \`content_zh_c_n\`;`)
  await db.run(sql`ALTER TABLE \`blogs\` DROP COLUMN \`legacy_content_zh_c_n\`;`)
  await db.run(sql`ALTER TABLE \`blogs\` DROP COLUMN \`slug_name_zh_t_w\`;`)
  await db.run(sql`ALTER TABLE \`blogs\` DROP COLUMN \`title_zh_t_w\`;`)
  await db.run(sql`ALTER TABLE \`blogs\` DROP COLUMN \`excerpt_zh_t_w\`;`)
  await db.run(sql`ALTER TABLE \`blogs\` DROP COLUMN \`content_zh_t_w\`;`)
  await db.run(sql`ALTER TABLE \`blogs\` DROP COLUMN \`legacy_content_zh_t_w\`;`)
}

export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`media_locales\`;`)
  await db.run(sql`DROP TABLE \`installations_locales\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "media_id", "installations_id", "blogs_id") SELECT "id", "order", "parent_id", "path", "users_id", "media_id", "installations_id", "blogs_id" FROM \`payload_locked_documents_rels\`;`)
  ...
  await db.run(sql`ALTER TABLE \`media\` ADD \`alt_en\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`media\` ADD \`alt_zh_c_n\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`media\` ADD \`alt_zh_t_w\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`installations\` ADD \`title_en\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`installations\` ADD \`location_en\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`installations\` ADD \`description_en\` text;`)
  await db.run(sql`ALTER TABLE \`installations\` ADD \`title_zh_c_n\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`installations\` ADD \`location_zh_c_n\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`installations\` ADD \`description_zh_c_n\` text;`)
  await db.run(sql`ALTER TABLE \`installations\` ADD \`title_zh_t_w\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`installations\` ADD \`location_zh_t_w\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`installations\` ADD \`description_zh_t_w\` text;`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`slug_name_en\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`title_en\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`excerpt_en\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`content_en\` text;`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`legacy_content_en\` text;`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`slug_name_zh_c_n\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`title_zh_c_n\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`excerpt_zh_c_n\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`content_zh_c_n\` text;`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`legacy_content_zh_c_n\` text;`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`slug_name_zh_t_w\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`title_zh_t_w\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`excerpt_zh_t_w\` text NOT NULL;`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`content_zh_t_w\` text;`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`legacy_content_zh_t_w\` text;`)
}
