  ...
  return (
    <section className="section" style={{ backgroundColor: '#ffffff' }}>
      <div className={`container-main ${maxWidthClass[maxWidth]}`}>
        <figure
          className={`relative overflow-hidden ${aspectRatioClass[aspectRatio]} ${roundedClass[rounded]} ${shadow ? 'shadow-xl' : ''}`}
        >
          <Image src={src} alt={alt} fill className="object-cover" />
        </figure>
        {caption && (
          <figcaption className="text-center mt-4 text-body-sm" style={{ color: '#a8a29e' }}>
            {caption}
          </figcaption>
        )}
      </div>
