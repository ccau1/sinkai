...
ssh root@YOUR_SERVER_IP
chmod 600 /opt/sinkai/ssl/cloudflare-origin.key
chmod 644 /opt/sinkai/ssl/cloudflare-origin.pem
```
...
For a static Next.js export, this is usually empty or minimal:

```bash
# Add any runtime env vars if needed
...
4. Pull the image and run `docker compose up -d`
5. Run a health check against `https://sinkai.tribalorigin.com`
